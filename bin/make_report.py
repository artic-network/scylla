...
def get_binned_data(list_vals, num_bins, start=0):
    ...
    for i in range(num_bins):
        binned_data.append(
            {
                "bin_start": start + step * i,
                "bin_end": start + step * (i + 1),
                "count": 0,
            }
        )
    for j in list_vals:
...
def main():
    ...
    parser.add_argument(
        "--warnings",
        help="text file containing any warnings",
        required=False,
        nargs="*",
        type=Path,
    )
    ...
    parser.add_argument(
        ...
        help="Database used for classification",
        default="PlusPF",
    )
    ...
    if args.read_counts:
        read_length_counts, read_length_step, read_quality_counts = (
            summarize_read_counts(args.read_counts)
        )
    else:
    ...
    data_for_report = {
        "sankey_data": assignments,
        "read_length_data": read_length_counts,
        "read_length_step": read_length_step,
        "read_quality_data": read_quality_counts,
